...
function T.retail_scene_realizes_generated_assets_and_changes_across_choice_flow(scope, context)
  ...
  for _, versionId in ipairs(versions) do
    ...
    local initial = drawFrame(scope, host, window, 520, 192)
    ...
    host:move("right")
    host:update()
    local rotating = drawFrame(scope, host, window, 520, 192)
    Assert.isTrue(
    ...
    local rotated = drawFrame(scope, host, window, 520, 192)
    ...
    local zoomed = drawFrame(scope, host, window, 520, 192)
    Assert.isTrue(
...
function T.non_trio_candidate_inspects_through_the_mon_portrait_contract(scope, context)
  ...
  for _, versionId in ipairs(versions) do
    ...
    local backend = prepareHost(host, cacheFs)
    local window = openWindowBorrower(cacheFs, versionId)
    local middleFrame = drawFrame(scope, host, window, 520, 192)
    Assert.isTrue(brightPixels(middleFrame, 520, 192) > 0, versionId .. " portrait is visible")

    host:focus(0)
    local neighboringFrame = drawFrame(scope, host, window, 520, 192)
    Assert.isTrue(
