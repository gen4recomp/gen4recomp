...
function T.transition_reveals_without_hiding_captured_partner()
  ...
  local ok, err = pcall(function()
    ...
    Assert.isTrue(actors:isVisible(PARTNER_ID), "the captured partner stays visible at transition start")
    transitions:updateFixed()
    Assert.isTrue(actors:isVisible(PARTNER_ID), "the partner stays visible through the prelude")
    transitions:updateFixed()
...
function T.stale_transition_does_not_mutate_replacement_partner()
  ...
  local ok, err = pcall(function()
    ...
    local transitions = transition(actors)
    Assert.isTrue(transitions:start(), "a live partner starts one instance")
    Assert.isTrue(captured:isVisible(), "the captured generation stays visible before replacement")
    ...
    Assert.isTrue(replacement:isVisible(), "the replacement keeps ordinary visible installation")
    transitions:updateFixed()
