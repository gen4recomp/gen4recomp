...
function T.partner_initial_visibility_is_install_only()
  local mgr = manager()
  mgr:installPartner(spec(nil, { initiallyVisible = false }))
  Assert.isFalse(mgr:isVisible("field:partner"), "an install may establish a hidden partner")
  mgr:updatePartner(spec(nil, { visualId = 20154, initiallyVisible = true }))
  Assert.isFalse(mgr:isVisible("field:partner"), "replacement preserves the installed visibility")
  mgr:dispose()
end
