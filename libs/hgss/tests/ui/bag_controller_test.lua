...
function T.tab_navigation_switches_pockets_and_keeps_tab_focus()
  local bag = stockTwoPockets(service())
  local cursor = BagCursor.new()
  cursor:setPocket("balls")
  local control = controller(bag, cursor)
  ...
  Assert.equal(control:status().focus, "tabs", "tabs right keeps tab focus")
  ...
  Assert.equal(control:status().focus, "tabs", "tabs left keeps tab focus")
  Assert.equal(cursor:position("balls"), 1, "returning restores the remembered position")
  Assert.equal(selectedKey(control:status()), "GREAT_BALL")
  control:updateFixed({ navigate("left") })
  Assert.equal(cursor:currentPocket(), "medicine", "tabs left steps back")
  Assert.equal(control:status().focus, "tabs", "tabs left still keeps tab focus")
  control:updateFixed({ navigate("left") })
  Assert.equal(cursor:currentPocket(), "items", "tabs left reaches the first pocket")
  Assert.equal(control:status().focus, "tabs", "tabs focus survives every switch")
  control:updateFixed({ navigate("left") })
  Assert.equal(cursor:currentPocket(), "key_items", "tabs left past the first pocket wraps")
  Assert.equal(control:status().focus, "tabs", "tabs focus survives the wrap")
end
